"""
pyasm2 - x86 assembler library          
(C) 2012 Jurriaan Bremer
Improved by Dimitri Fourny
"""
import struct
import types
...
class MemoryAddress:
    ...
    def __str__(self):
        ...
        if hasattr(self.disp, 'value') and self.disp.value:
            q = '0x%x' % int(self.disp)
            s += q if not len(s) else '+' + q
        if s == '':
            s = '0x%x' % int(self.disp)
...
class Instruction:
    ...
    VALID_OPERANDS = (int, long, SegmentRegister, GeneralPurposeRegister,
                      MemoryAddress, Immediate, XmmRegister, list, Label)
    ...
    def encoding(self):
        ...
        raise Exception('Unknown or Invalid Encoding: %s %s %s' % (self.__class__.__name__, self.op1.__class__.__name__, self.op2.__class__.__name__))

    def name(self):
        """The name of this instruction."""
        return self._name_ or self.__class__.__name__

    def get_reg_dependencies(self):
        # op1
        if isinstance(self.op1, GeneralPurposeRegister):
            self.dependencies.append(self.op1)
        elif isinstance(self.op1, MemoryAddress):
            self.dependencies.append(self.op1.reg1)
            self.dependencies.append(self.op1.reg2)
        # op2
        if isinstance(self.op2, GeneralPurposeRegister):
            self.dependencies.append(self.op2)
        elif isinstance(self.op2, MemoryAddress):
            self.dependencies.append(self.op2.reg1)
            self.dependencies.append(self.op2.reg2)
        # op3
        if isinstance(self.op3, GeneralPurposeRegister):
            self.dependencies.append(self.op3)
        elif isinstance(self.op3, MemoryAddress):
            self.dependencies.append(self.op3.reg1)
            self.dependencies.append(self.op3.reg2)

        self.dependencies = list(set(filter(None, self.dependencies))) # delete None and duplicates
    ...
    def resolve_labels(self, local_labels):
        if isinstance(self.op1, Label): 
            self.op1 = Immediate(local_labels[self.op1.index])
        if isinstance(self.op2, Label):
            self.op2 = Immediate(local_labels[self.op2.index])
...
class Block:
    ...
    def assemble(self, recursion=10):
        ...
        for idx, instr in enumerate(self._l):
            if hasattr(instr, 'resolve_labels'):
                instr.resolve_labels(local_labels) # resolve labels if needed
    ...
    def get_reg_dependencies(self):
        ...
        for inst in self._l:
            if hasattr(inst, 'get_reg_dependencies'):
                dependencies += inst.get_reg_dependencies()

        dependencies = list(set(filter(None, dependencies))) # delete None and duplicates
        return dependencies
...
class mov(Instruction):
    ...
    _enc_ = \
        ...
        zip(range(0xb8, 0xc0), gpr.register32, ((dword, imm),) * 8) + [
            ...
            ('\x66\xc7', (word, memgpr, 0), (word, imm)),
        ]
...
class inc(Instruction):
    ...
    _enc_ += zip(['\x66'+str(elt) for elt in range(0x40, 0x48)], gpr.register16) 
    _enc_ = [
        (0xfe, (byte, memgpr, 0)),
        (0xff, (dword, memgpr, 0)),
        ('\x66\xff', (word, memgpr, 0))]
...
class jmp_abs(Instruction):
    _enc_ = [
        (0xFF, (dword, memgpr, 4)),
        ('\x66\xFF', (word, memgpr, 4)),
    ]    

def call(arg):
    return _branch_instr('call', 0xe8, None, arg)

class call_abs(Instruction):
    _enc_ = [
        (0xFF, (dword, memgpr, 2)),
        ('\x66\xFF', (word, memgpr, 2)),
    ]    

class je_rel(Instruction):
    _enc_ = [
        (0x74, (byte, memgpr)),
    ]    
...
_group_1_opcodes = lambda x: [
    ...
    ('\x66'+str(0x01+8*x), (word, memgpr), (word, gpr)),
    ...
    ('\x66\x81', (word, memgpr, x), (word, imm))]
...
class sub(Instruction):
    _enc_ = [
        ...
        ('\x66\x29', (word, memgpr), (word, gpr)),
        (0x2A, (byte, gpr), (byte, memgpr)),
        (0x81, (dword, memgpr, 5), (dword, imm)),
        ('\x66\x81', (word, memgpr, 5), (word, imm)),
        (0x82, (byte, memgpr, 5), (byte, imm)),
        (0x83, (dword, memgpr, 5), (byte, imm)),
        ('\x66\x83', (word, memgpr, 5), (byte, imm)),
    ]
